...
class Aggregator:
    ...
    def __init__(self,
        ...
        if LOG_WANDB:
            wandb.init(project="LBL QSKEW FEDERATED ROCKET 1000", entity="mlgroup", group=f"{my_aggregator_name}", tags=["Federated", "TCS"],
                       config={
                           "num_clients": 4,
                           "rounds": 100
                       },
    ...
    def send_local_task_results(self, collaborator_name, round_number, task_name,
        ...
        for named_tensor in named_tensors:
            ...
            if 'metric' in tensor_key.tags:
                ...
                #if LOG_WANDB:
                    #wandb.log({f"{task_name} {tensor_key.tensor_name}": metric_value})
        ...
        for tensor_key in self.collaborator_tasks_results[task_key]:
            ...
            if report:
                ...
                if agg_function:
                    ...
                    self.logger.metric(f'TASK NAME: {task_name}')
                    self.logger.metric(f'AGG_TENSOR_NAME: {agg_tensor_name}')
                    self.logger.metric(f'AGG_RESULTS: {agg_results:f}')
                    if LOG_WANDB:
                ...
                self.metric_queue.put(metric_dict)
                #if LOG_WANDB:
                    #wandb.log({f"{task_name}": float(metric_dict["metric_value"])})
